...
class XNovaPageDownload:
    ...
    def download(self, page_name: str):
        ...
        if page_name == 'overview':
            page_content = self.internal_download_url('?set=overview')
        else:
            raise ValueError('XNovaPageDownload: unknown page type requested: {0}'.format(page_name))
        return page_content

    def internal_set_error(self, errstr):
        pass

    def internal_download_url(self, url_append: str):
        ...
        try:
            ...
            if r.status_code == requests.codes.ok:
                ...
                self.internal_set_error('HTTP {0}'.format(r.status_code))
        except requests.exceptions.RequestException as e:
            logger.debug('Exception {0}'.format(type(e)))
            self.internal_set_error(str(e))
        return text
